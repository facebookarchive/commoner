    ...
    writeBundleP: function(bundleP) {
        ...
        return Q.resolve(bundleP).then(function(bundle) {
            var hash = createHash("sha1")
                ...
                .update(writer.salt)
                .digest("hex")
                .slice(0, BUNDLE_HASH_LENGTH);

            var slug = util.idToSlug(bundle.rootId);
            var fileName = [slug, hash, "js"].join(".");

            return writer.writeFileP(bundle, fileName);
        });
    },

    writeFileP: util.cachedMethod(function(bundle, fileName) {
        var writer = this;
        ...
        return util.existsP(fullPath).then(function(exists) {
            ...
            return promise.then(function(source) {
                return util.writeP(fullPath, source).then(function() {
                    util.log.err("built " + bundle, "green");
                    return fileName;
                });
            });
        });
