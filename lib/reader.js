...
function ModuleReader(sourceDir) {
    ...
    Object.defineProperties(self, {
        resolver: { value: new Resolver(sourceDir) },
...
ModuleReader.prototype = {
    ...
    noCacheReadModuleP: function(givenId) {
        var reader = this;
        assert.strictEqual(typeof givenId, "string");

        return reader.resolver.resolveP(givenId).then(function(result) {
            assert.ok(result, "could not resolve module " + givenId);
            ...
            var context;
            var promise = readFileP(result.path).then(function(source) {
