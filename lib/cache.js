    ...
    return this.cacheDirP.then(function(cacheDir) {
        ...
        return util.openExclusiveP(file).then(function(fd) {
            ...
            return stringP.then(function(string) {
                ...
                // Note that util.writeFdP closes the file descriptor
                // under normal circumstances.
                fs.closeSync(fd);

                // If stringP resolved to an error, remove the cache file
                // so we don't reuse it blindly in the future.
                return util.unlinkP(file).then(function() {
                    throw err;
                });
            });

        }, function(err) {
            // Note that this failure handler applies only to the
            // util.openExclusiveP promise.

            if (err.code === "EEXIST") {
                // If opening the file with -x failed because the file
                // already existed, then return its existing contents.
